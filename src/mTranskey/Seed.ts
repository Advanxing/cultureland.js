...
export class Seed {
    ...
    public static EncRoundKeyUpdate0(K: number[], A: number[], B: number[], C: number[], D: number[], Z: number) {
        ...
        A[0] = (A[0] >>> 8) ^ (B[0] << 24);
        B[0] = (B[0] >>> 8) ^ (a << 24);
        const b = A[0] + C[0] - KC[Z];
        const c = B[0] + KC[Z] - D[0];
        K[0] = SS0[Seed.GetB0(b)] ^ SS1[Seed.GetB1(b)] ^ SS2[Seed.GetB2(b)] ^ SS3[Seed.GetB3(b)];
        K[1] = SS0[Seed.GetB0(c)] ^ SS1[Seed.GetB1(c)] ^ SS2[Seed.GetB2(c)] ^ SS3[Seed.GetB3(c)];
    }
    ...
    public static SeedEncryptCbc(a: number[], b: number[], c: number[], d: number, e: number[]) {
        ...
        const f = new Array(blockSize * 16 + remainLen);
    ...
    public static SeedDecryptCbc(a: number[], b: number[], c: number[], d: number, e: number[]) {
        ...
        const blockSize = d / 16;
        const remainLen = d % 16;
        const f = new Array(16 * blockSize);
        const g = new Array(16);
        const h = new Array(16);
        const j = new Array(16);
        Seed.ArrayCopy(b, 0, j, 0, 16);
    ...
    public static SeedEnc(geo: string, sessionKey: Uint8Array) {
        ...
        const roundKey: number[] = new Array(32);
        ...
        return outData.map(c => c.toString(16)).join(",");
    }
}
